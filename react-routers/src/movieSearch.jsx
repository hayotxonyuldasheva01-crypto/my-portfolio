...
function MovieSearch() {
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial',backgroundColor:'#fff', borderRadius:'30px'}}>
      ...
      {/* Qidiruv qismi */}
      <form onSubmit={searchMovies} style={{ display: 'flex', gap: '10px', marginBottom: '30px',
      ...
      {selectedMovie && (
        <div style={{
            position:'fixed', top:0,left:0,width:'100%', height:'100%', backgroundColor:'rgba(0,0,0,0.8)',display:'flex',justifyContent:'center',alignItems:'center', zIndex:1000
        }}>
            <div style={{
                backgroundColor:'white',padding:'20px',borderRadius:"15px",maxWidth:'600px',width:'90%',position:'relative',display:'flex',gap:'20px'
            }}>
                <button onClick={()=>setSelectedMovie(null)} style={{position:'absolute',top:'10px',right:"15px",fontSize:"20px", cursor:'pointer', border:"none",background:'none'}}>X</button>
                ...
                <div>
                    ...
                    <p><strong>Reyring</strong>{selectedMovie.imdbRating}</p>
